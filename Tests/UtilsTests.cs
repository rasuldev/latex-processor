...
связанных с решением ряда актуальных современных задач, возникающих в таких областях, как обработка и сжатие временных рядов и изображений 
\cite{1,2,3,4}, приближенное решение систем нелинейных дифференциальных  и разностных уравнений численно-аналитическими методами 
\cite{5,6,7,8}, численное обращение преобразования Радона 
\cite{9,10},  идентификация линейных и нелинейных систем автоматического регулирования и управления 
\cite{11,12}  и других. (Числа в квадратных скобках означают ссылки на 
литературу, приведенную для удобства чтения в конце настоящего пункта.)
\cite[opt]{13}

";
            var cites = Utils.GetCites(text);
            Assert.That(cites.Count == 5);
            ...
            Assert.AreEqual(1, cites[4].Keys.Count);
            Assert.AreEqual("opt", cites[4].OptionalParam);
